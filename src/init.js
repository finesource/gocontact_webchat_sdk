(function () {
    ...
    if ((isInMatchingPag && FSWebChat[domain_hash]) || webchatdialogstatus) {
        ...
        if (!FSWebChat._pagecontainer || FSWebChat._pagecontainer.length === 0) {
            document.body.appendChild(div);
        } else {
            var divContainer = document.createElement('div');
            divContainer.id = FSWebChat._pagecontainer;
            divContainer.style.display = 'none';
            FSWebChat._pageContainer = divContainer;
            divContainer.appendChild(div);
            document.body.appendChild(divContainer);
        }

        if (FSWebChat._externalform && FSWebChat._externalform.hasOwnProperty('url') && FSWebChat._externalform.hasOwnProperty('style')) {
            var divExtForm = document.createElement('div');
            divExtForm.id = `${FSWebChat._divid}_ext_form`;
            divExtForm.style.cssText = FSWebChat._externalform.style;
            Object.assign(divExtForm.style,{transition: "all 0.2s linear 0s", zIndex: 2147483647, display: "none",position: "fixed", overflow: "hidden"});


            var iframeExtForm = document.createElement('iframe');
            iframeExtForm.id = `${FSWebChat._ifid}_ext_form`;
            Object.assign(iframeExtForm.style,{width: "100%", height: "100%"});

            try{
                let domainEF = (new URL(FSWebChat._externalform.url));
                FSWebChat._extform_server = domainEF.origin;
            }catch(e){
                console.error(e);
                FSWebChat._extform_server = FSWebChat._file_server;
            }
            iframeExtForm.setAttribute('src', FSWebChat._externalform.url);
            iframeExtForm.setAttribute('frameBorder', "0");
            iframeExtForm.addEventListener('load', function() {
                console.warn('External Form loaded');
                FSWebChat.initExtFormComunication();
            });
            divExtForm.appendChild(iframeExtForm);
            FSWebChat._extFormContainer = divExtForm;
            document.body.appendChild(divExtForm);
        }
    } else {
    ...
    function FSWebChathandleResponse(e) {
        ...
        var isServer = (e.origin.indexOf(FSWebChat._file_server) !== -1);
        var isExtForm = (FSWebChat._extform_server && e.origin.indexOf(FSWebChat._extform_server) !== -1);
        if(isServer === false && isExtForm=== false){
            return false;
        }
        ...
        if (e.data.action === 'RemoveElement') {
            if (e.data.elem !== '') {
                var element = document.getElementById(e.data.elem);
                if (element) {
        ...
        if (e.data.action === 'ShowHideElement') {
            if (e.data.elem !== '') {
                var element = document.getElementById(e.data.elem);
                if (element) {
        ...
        if (e.data.action === 'ShowHidePageContainer') {
            if (FSWebChat._pageContainer) {
                var element = document.getElementById(e.data.elem);
                FSWebChat._pageContainer.style.display = e.data.display;
            }
            return;
        }
        if (e.data.action === 'ShowHideExtFormContainer') {
            if (FSWebChat._extFormContainer) {
                var element = document.getElementById(e.data.elem);
                FSWebChat._extFormContainer.style.display = e.data.display;
            }
            return;
        }
        if (e.data.action === 'StartMirroring') {
            try {
        ...
        if (e.data.action === 'StopMirroring') {
            try {
                localStorage.setItem("_statusvarofwcview", false);
                FSWebChat.iframe.contentWindow.postMessage({"action": 'SetStatusVarWCView', "params": {status: localStorage.getItem("_statusvarofwcview")}}, FSWebChat._file_server);
            } catch (e) {
        ...
        if (e.data.action === 'GetClientBrowserURL') {
            FSWebChat.sendBrowserPath();
            return;
        }
        if (e.data.action === 'GetStatusVarWCView') {
            ...
            if(FSWebChat._usingExtForm ===  true){
                FSWebChat.informUsingExtForm();
            }
            return;
        }
        if (e.data.action === 'ResetStatusVarWCView') {
            localStorage.setItem("_statusvarofwcview", false);
            return;
        }
        if (e.data.action === 'SetStatusVarWCInactivity') {
            localStorage.setItem("_statusvarofwcinactivity", 'resetClient');
            return;
        }
        if (e.data.action === 'getClientGeolocation') {
        ...
        if (e.data.action === 'WebChatDialogStatus') {
            localStorage.setItem("_webchatdialogstatus", e.data.status);
            return;
        }
        if (e.data.action === 'ExtFormSetExtraFields') {
            FSWebChat.SetExtraFields(e.data.object);
            return;
        }
        if (e.data.action === 'ExtFormStartConversation') {
            FSWebChat.StartConversation(e.data.object);
            return;
        }
        if (e.data.action === 'ExtFormHideExtForm') {
            FSWebChat.showHideExtForm('hide');
            return;
        }

        if (e.data.action === 'FSWebChatExtFormLoaded') {
            FSWebChat._usingExtForm =  true;
            FSWebChat.informUsingExtForm();
            return;
        }
    ...
    //*********************************************************************************
    //                            IFRAME COMUNICATION ExternalForm
    //*********************************************************************************
    FSWebChat.initExtFormComunication = function(){
        var extrenalForm = document.getElementById(`${FSWebChat._ifid}_ext_form`);
        if(!extrenalForm){
            console.warn('Element ExternalForm not found');
            return false;
        }
        FSWebChat.iframeExtForm = extrenalForm;
        FSWebChat.iframeExtForm.contentWindow.postMessage({
            "action": 'setOrigin',
            "params": {
                "originPath": location.pathname
            }
        }, FSWebChat._extform_server);
    }
    ...
    FSWebChat.setOrigin = function () {
        FSWebChat.iframe.contentWindow.postMessage({
            ...
            "params": {
                ...
                "_endpoint_rest": FSWebChat._endpoint_rest || FSWebChat._service,
    ...
    FSWebChat.StartConversation = function (fieldMap) {
        FSWebChat.iframe.contentWindow.postMessage({
            action: 'Control:StartConversation',
            fieldMap: fieldMap
        }, FSWebChat._file_server);
    };

    FSWebChat.SetExtraFields = function (extraFields) {
        FSWebChat.iframe.contentWindow.postMessage({
            action: 'Control:SetExtraFields',
            extraFields: extraFields
        }, FSWebChat._file_server);
    };

    FSWebChat.informUsingExtForm = function () {
        FSWebChat.iframe.contentWindow.postMessage({
            action: 'Inform:UsingExtForm'
        }, FSWebChat._file_server);
    };

    FSWebChat.showHideExtForm = function (_display) {
        if (FSWebChat._extFormContainer) {
            var display = _display === 'show' ? '' : 'none';
            FSWebChat._extFormContainer.style.display = display;
        }
    };
